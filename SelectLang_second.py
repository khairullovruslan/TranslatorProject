...
class SecondForm_2(QMainWindow):
    def __init__(self):
        ...
        uic.loadUi('SecondForm_2.ui', self)

        # TODO Styles
        self.list_of_languages.setStyleSheet(
            "QComboBox {background-image: url(select_2.jpg); padding:25px 50px 30px 155px;"
            "border-radius: 10px; color: rgb(255, 255, 255);}"
            "QComboBox::drop-down{border-radius: 10px; background-image: url(select_2.jpg);"
        ...
        self.history_lang.setStyleSheet(
            "background-image: url(select_2.jpg); border-radius: 10px; color: rgb(255, 255, 255);")
        ...
        self.contact_vk.clicked.connect(
            lambda: webbrowser.open('https://vk.com/im?peers=363207223_592456333_458503653_-97568278&sel=262014370'))
        self.contact_tg.clicked.connect(
            lambda: webbrowser.open('https://t.me/I_am_a_busy_person'))
        self.contact_yandex.clicked.connect(
        ...
        languages_dict = {'Русский': 'Russian', 'Английский': 'English', 'Испанский': 'Spanish',
                          'Французский': 'French',
                          'Бенгальский': 'Bengali', 'Португальский': 'Portuguese', 'Индонезийский': 'Indonesian',
                          'Японский': 'Japanese', 'Турецкий': 'Turkish', 'Украинский': 'Ukrainian',
                          'Корейский': 'Korean',
        ...
        self.contact_yandex.setCursor(QCursor(QtCore.Qt.PointingHandCursor))

        # TODO СОЗДАНИЕ ВСПЛЫВАЮЩИХ ОКОН
        self.contact_tg.setToolTip('Перейти')
        self.contact_vk.setToolTip('Перейти')
        self.contact_yandex.setToolTip('Перейти')
    ...
    def open_second_form(self):  # TODO ОТКРЫТИЕ ВТОРОГО ОКНА
        ...
        self.history_form.setWindowIcon(QtGui.QIcon('logo.png'))
